 ...
 *
 * Replaces the old native <iframe src={pdf}> preview, which on iOS Safari and
 * the LINE in-app browser (WKWebView) renders a PDF as a single, static,
 * non-scrollable snapshot of page 1 — the reported bug. pdf.js renders every
 * page to its own <canvas> with JS, so behaviour is identical across browsers:
 * all pages visible, fit-to-width, vertical scroll inside the modal body.
 *
 * The PDF bytes come from a cross-origin R2 presigned URL, so the fetch below
 * requires CORS on the R2 bucket (Access-Control-Allow-Origin for the web
 * origin). Fetching the whole file as an ArrayBuffer (rather than letting
 * pdf.js do ranged requests) keeps that CORS requirement minimal — no need to
 * expose Content-Range / Accept-Ranges headers.
 */
  ...
  if (status === 'error') {
    return (
      <div className="preview-fallback">
        ...
        <button className="btn" onClick={() => window.open(url, '_blank', 'noopener')}>
          เปิดไฟล์ในแท็บใหม่
        </button>
      </div>
    );
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="preview-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`preview-body${isPdf && url ? ' preview-body-pdf' : ''}`}>
          {loading ? (
            ...
            // FIX: pdf - render all pages via pdf.js instead of a native <iframe>,
            // which shows only page 1 (non-scrollable) on iOS / the LINE browser.
