 ...
 *
 * The bytes are fetched SAME-ORIGIN from /api/file-pdf/:id (a Next route handler
 * that streams the R2 object server-side). That is deliberate: fetching the
 * cross-origin presigned R2 URL from the browser would need R2 bucket CORS — an
 * infra setting not in this repo — and is what made two earlier attempts fall
 * back to "open in new tab" on real devices. Same-origin needs neither CORS nor
 * a CSP connect-src relaxation.
 *
 * We import the LEGACY pdf.js build: the modern bundle uses
 * `Promise.withResolvers` (Safari 17.4+), which throws on the older iOS / LINE
 * WKWebView versions this viewer exists to support. The worker (copied to
 * public/ by scripts/copy-pdf-worker.mjs) is the matching legacy worker.
 */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Same-origin stream (see the component doc comment). Sends the session
        // cookie automatically; the route re-checks access server-side.
        const res = await fetch(`/api/file-pdf/${encodeURIComponent(fileId)}`);
  ...
  if (status === 'error') {
    return (
      <div className="preview-fallback">
        ...
        <a
          className="btn"
          href={`/api/file-pdf/${encodeURIComponent(fileId)}`}
          target="_blank"
      ...
      <div className="preview-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className={`preview-body${usePdfViewer && url ? ' preview-body-pdf' : ''}`}>
          {loading ? (
            ...
            // <iframe> below shows only a static, non-scrollable page 1 on iOS /
            // the LINE in-app browser (WKWebView). Desktop keeps the iframe.
            <PdfViewer key={file.id} fileId={file.id} />
          ) : inline && url ? (
            // Desktop PDFs and all text files render in a native browser viewer
            // via <iframe> — this gives the browser's built-in PDF UI (page
            // thumbnails, zoom, all pages). Mobile PDFs take the pdf.js branch
            // above; do NOT route them here (iOS/WKWebView can't scroll it).
