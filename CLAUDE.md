...
| `REMINDER_POLICY[plan].maxSelectable` (free 1 / pro 2 / premium 4) | how many of the **13** §4b lead times may be ticked. Every plan sees the same menu — a plan caps the COUNT, never which ones. Enforced in `resolveReminderConfig` (403 `REMINDER_INTERVAL_LIMIT`); the picker's own cap is a courtesy | `config/plans.ts`, `middleware/planGuard.ts`, `components/ReminderPicker.tsx` + `ReminderSheet.tsx` |
